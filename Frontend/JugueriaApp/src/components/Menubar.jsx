    ...
    const sendVerificationOtp = async () => {
        try {
            ...
            if(response.status === 200) {
                ...
                toast.success("El OTP ha sido enviado de manera exitosa");
            } else {
                toast.error("No se ha podido enviar el OTP");
            }
        } catch (error) {
            toast.error(error.response.data.message);
        }
    ...
    return (
        <nav className="navbar bg-white px-5 py-4 d-flex justify-content-between align-items-center">
            <div className="d-flex align-items-center gap-2">
                ...
                <span className="fw-bold fs-4 text-dark">Juguería Tía Julia</span>
            </div>
            ...
            {userData ? (
                ...
                <div className="btn btn-outline-dark rounded-pill px-3" onClick={() => navigate('/login')  }>
                    Inicie Sesión <i className="bi bi-arrow-right ms-2"></i>
                </div>
            )}
